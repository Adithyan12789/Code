...
exports.getDailyRewardCoinByUser = async (req, res) => {
  try {
    ...
    const [user, userCheckIn, dailyRewards] = await Promise.all([User.findOne({ _id: req.query.userId }), CheckIn.findOne({ userId: req.query.userId }), DailyRewardCoin.find({}).sort({ day: 1 })]);
