...
exports.fetchNewReleasesForUser = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      status: true,
      message: "Retrieved new release data for the user.",
...
exports.getMoviesGroupedByCategory = async (req, res) => {
  try {
    ...
    const baseUrl = process.env.WASABI_URL || `${req.protocol}://${req.get("host")}`;
    const formattedGroupedMovies = groupedMovies.map((group) => ({
      ...group,
    ...
    return res.status(200).json({
      ...
      groupedMovies: formattedGroupedMovies,
    });
  } catch (error) {
    console.log(error);
...
exports.fetchMoviesSeries = async (req, res) => {
  try {
    const movieSeries = await MovieSeries.find({ isActive: true, isAutoAnimateBanner: true })
      .select("thumbnail banner name")
      .lean();

    const baseUrl = process.env.WASABI_URL || `${req.protocol}://${req.get("host")}`;
    const formattedData = movieSeries.map((item) => ({
      ...item,
      thumbnail: item.thumbnail ? `${baseUrl}/admin/NewsChannelPoster${item.thumbnail}` : null,
      banner: item.banner ? `${baseUrl}/admin/NewsChannelBanner${item.banner}` : null,
    }));
  ...
  try {
    ...
    if (req.query.userId) {
        ...
        MovieSeries.aggregate([
          { $match: { isActive: true } },
          {
            $lookup: {
              ...
              pipeline: [
                { $match: { userId: userId, $expr: { $in: ["$$movieSeriesId", "$videos.movieSeries"] } } },
                { $project: { _id: 1 } },
              ],
              as: "userVideoList",
            },
          },
          { $addFields: { isAddedToList: { $gt: [{ $size: "$userVideoList" }, 0] } } },
          {
            $project: {
              _id: 1,
              releaseDate: 1,
      ...
      const videos = await MovieSeries.aggregate([
        { $match: { isActive: true } },
        { $addFields: { isAddedToList: false } },
        { $project: { _id: 1, releaseDate: 1, name: 1, description: 1, thumbnail: 1, isAddedToList: 1 } },
        { $sort: { releaseDate: -1 } },
        { $skip: (start - 1) * limit },
        { $limit: limit },
      ]);
...
exports.fetchMoviesGroupedByGenre = async (req, res) => {
  try {
    ...
    const [categoryIds, user] = await Promise.all([
      Category.find().limit(8).distinct("_id").lean(),
      userId ? User.findOne({ _id: userId }).select("isBlock").lean() : null
    ]);
      ...
      { $match: { isActive: true, category: { $in: categoryIds } } },
      {
        $lookup: {
          ...
          pipeline: [
            { $match: { userId: userId } },
            { $unwind: "$videos" },
            { $match: { $expr: { $eq: ["$videos.movieSeries", "$$movieId"] } } },
            { $project: { _id: 1 } }
          ],
          as: "userVideo",
        },
        ...
        $group: {
          ...
          movies: {
            $push: {
              ...
              thumbnail: "$thumbnail",
              isAddedToList: { $gt: [{ $size: "$userVideo" }, 0] },
            },
          },
        },
      },
...
exports.fetchMediaCollection = async (req, res) => {
  try {
    ...
    if (userId && !mongoose.Types.ObjectId.isValid(userId)) {
      return res.status(200).json({ status: false, message: "Invalid userId format. It must be a valid ObjectId." });
    }
    ...
    const moviesQuery = MovieSeries.aggregate([
      ...
      { $unwind: { path: "$category", preserveNullAndEmptyArrays: true } },
      { $lookup: { from: "watchhistories", localField: "_id", foreignField: "movieSeries", as: "watchData" } },
      { $addFields: { views: { $size: "$watchData" } } },
      ...(userId
        ? [
            {
              $lookup: {
                ...
                pipeline: [
                  { $match: { userId: userId } },
                  { $project: { isAddedToList: { $in: ["$$movieSeriesId", "$videos.movieSeries"] } } },
                ],
                as: "userVideoList",
              },
            },
            {
              $set: {
                isAddedToList: { $cond: { if: { $gt: [{ $size: "$userVideoList" }, 0] }, then: { $arrayElemAt: ["$userVideoList.isAddedToList", 0] }, else: false } },
              },
            },
            { $unset: "userVideoList" },
          ]
      ...
      { $sort: { views: -1, isTrending: -1, releaseDate: -1 } },
      { $skip: (start - 1) * limit },
      { $limit: limit },
    ]);
