...
exports.createContent = async (req, res) => {
  try {
    ...
    if (!name || !description || !category || !thumbnail || !banner || !maxAdsForFreeView) {
      await Promise.all([
        req.body.thumbnail && deleteFromStorage(req.body.thumbnail),
        req.body.banner && deleteFromStorage(req.body.banner),
      ]);
    ...
    const responseData = newContent.toObject();
    responseData.thumbnail = `${baseUrl}${thumbnailPath}`;
    responseData.banner = `${baseUrl}${bannerPath}`;
...
exports.updateContent = async (req, res) => {
  try {
    ...
    if (req?.body?.thumbnail) {
      ...
      movieWebseries.thumbnail = `/${req.body.thumbnail.split("/").pop()}`;
    }
    ...
    if (req?.body?.banner) {
      ...
      movieWebseries.banner = `/${req.body.banner.split("/").pop()}`;
    }
    ...
    const baseUrl = process.env.WASABI_URL || `${req.protocol}://${req.get("host")}`;
    const responseData = movieWebseries.toObject();
    responseData.thumbnail = responseData.thumbnail ? `${baseUrl}${responseData.thumbnail}` : null;
    responseData.banner = responseData.banner ? `${baseUrl}${responseData.banner}` : null;
...
exports.toggleTrendingStatus = async (req, res) => {
  try {
    ...
    if (!movieWebseries) {
      return res
        .status(200)
        .json({ status: false, message: "Movie or web series not found." });
    }
  ...
  try {
    if (!req.query.movieWebseriesId) {
      return res
        ...
        .json({
          status: false,
          message: "The 'movieWebseriesId' field is required.",
        });
    }

    const movieWebseriesId = new mongoose.Types.ObjectId(
    ...
    return res
      ...
      .json({
        status: true,
        message: `The Movie or web series has been updated successfully.`,
        data: movieWebseries,
      });
  } catch (error) {
    console.log(error);
  ...
  try {
    ...
    const [totalMovieSeries, movieWebseries] = await Promise.all([
      ...
      News.aggregate([
        ...
        { $addFields: { totalShortVideos: { $size: "$shortVideos" } } },
        { $project: { shortVideos: 0 } },
        {
          $lookup: {
            from: "categories",
            localField: "category",
    ...
    const formattedData = movieWebseries.map((item) => ({
      ...item,
...
exports.removeMovieSeries = async (req, res) => {
  try {
    ...
    res.status(200).json({
      status: true,
      message:
        "Movie/Web series and associated short videos deleted successfully.",
    });
