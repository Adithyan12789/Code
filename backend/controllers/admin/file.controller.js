...
exports.uploadContent = async (req, res) => {
  try {
    ...
    console.log("Single Upload started for admin side .......", req.file);
    ...
    if (activeStorage === "local") {
      ...
      url = `${settingJSON?.doEndpoint}/${req.file.originalname}`;
    } else if (activeStorage === "aws") {
      url = `${settingJSON.awsEndpoint}/${req.file.originalname}`;
    }
  ...
  try {
    ...
    req.files.forEach((file) => {
      ...
      if (activeStorage === "local") {
        ...
        fileUrl = `${settingJSON?.doEndpoint}/${file.originalname}`;
      } else if (activeStorage === "aws") {
        fileUrl = `${settingJSON.awsEndpoint}/${file.originalname}`;
      }
